...
//3. Is 2-14 chars
const isBetweenTwoAndFourteenChars = (str) =>
...
const firstNameErrorMessageElement = document.getElementById(
  "error-message-first-name"
);
const lastNameErrorMessageElement = document.getElementById(
  "error-message-last-name"
);
const emailErrorMessageElement = document.getElementById("error-message-email");
const formErrorMessage = document.getElementById("error-message-form");
...
const nameLengthError =
  "Names must be no less than 2 characters and no longer than 14 characters!";
...
const validatorMapping = {
  title: (str) =>
    (errorMessageElements["title"].textContent = titleValidator(str)),
  "first-name": (str) =>
    (errorMessageElements["first-name"].textContent = nameValidator(
      str,
      "first-name"
    )),
  "last-name": (str) =>
    (errorMessageElements["last-name"].textContent = nameValidator(
      str,
      "last-name"
    )),
  email: (str) =>
    (errorMessageElements["email"].textContent = emailValidator(str)),
};
...
const checkValidity = () => {
  ...
  Object.entries(validationState).forEach(([key, value]) => {
    ...
    submitButtonEl.disabled = !isAllValid;
    formErrorMessage.textContent = isAllValid
      ? " "
